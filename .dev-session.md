...
[2026-02-17 - Phase 7 COMPLETE]

Verification results (all passing):

- `curl -I https://turnout.network` → HTTP/2 200 ✅
  - x-powered-by: Next.js, x-cache: Hit from cloudfront
- `curl -I https://www.turnout.network` → HTTP/2 301 ✅
  - location: https://turnout.network/
- `pnpm prod:deploy` → Clean deploy, exit 0 ✅
  - Output: `TurnoutWeb: https://turnout.network`
  - Output: `web: https://turnout.network`, `cron: HelloCron`

All Phase 7 success criteria met.

[2026-02-17 - Phase 8 COMPLETE]

- Updated README.md with Production Deployment section
- Content matches TDD0000a Phase 8 specification exactly
- Includes: prerequisites, deploy command, verify steps, rollback, cache invalidation
- Commit: 0b612bc

[2026-02-17 - TDD0000a COMPLETE]

All success criteria met:

1. ✅ `https://turnout.network` loads with valid SSL (HTTP 200)
2. ✅ `https://www.turnout.network` redirects to apex (HTTP 301)
3. ✅ SSL certificate covers both apex and www
4. ✅ Production deployment uses custom domain (not CloudFront URL)
5. ✅ Dev stages still work with auto-generated URLs
6. ✅ Production deployment is repeatable via `pnpm prod:deploy`
7. ✅ Domain remains registered at Gandi (only nameservers changed)
8. ✅ README.md updated with production deployment documentation
